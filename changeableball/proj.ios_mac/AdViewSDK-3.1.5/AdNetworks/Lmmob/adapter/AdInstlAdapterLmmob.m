...
- (BOOL)loadAdInstl:(UIViewController *)controller {
    ...
    MBJoyView * lmmobView=[[MBJoyView alloc]  initWithAdUnitId:appID
                                                        adType:AdTypeMediumRectangle
                                            rootViewController:self.parent
                                                      userInfo:@{accountname: @"your user's app"}];
    self.immobInstl = lmmobView;
    self.immobInstl.delegate = self;
    [self.adInstlManager adapter:self requestString:@"req"];
    [self.immobInstl request];
...
{
     NSError *errorMessage = [NSError errorWithDomain:[NSString stringWithFormat:@"error:%ld",errorCode] code:errorCode userInfo:nil];
    [self.adInstlManager adapter:self requestString:@"fail"];
